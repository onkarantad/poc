...
WHILE 1 = 1
BEGIN
INSERT INTO SI_CORE_DWH_SSI_ABSA.ACCOUNTING_ENTRY_LINE_DETAILS_BATCH_LOAD_TEST WITH (TABLOCK) (ACCT_DET_ID,
	ACCT_DET_NUM,
...
--	(SELECT * FROM SI_IDIT_ODS.AC_ENTRY order by ID
--	OFFSET '?offset' ROWS FETCH NEXT '?batchSize' ROWS ONLY)AC_ENTRY
...
WHERE
NOT EXISTS
(SELECT 1 FROM SI_CORE_DWH_SSI_ABSA.ACCOUNTING_ENTRY_LINE_DETAILS_BATCH_LOAD_TEST WHERE ACCT_ENTRY_LINE_DET_ID =concat(AC_ENTRY.ID, '|', '101', '|', 'IDIT'))
